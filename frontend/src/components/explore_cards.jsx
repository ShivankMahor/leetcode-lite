function Card({ data }) {
  const { imageUrl, heading, subHeading, chapters, items, progress } = data;
  ...
  return (
    <div className="inline-block px-3">
      <div className="relative w-64 h-64 max-w-xs overflow-hidden rounded-lg shadow-md bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out">
        ...
        {/* Heading overlay */}
        <div className="absolute top-0 left-0 p-4 text-white drop-shadow-lg">
        ...
        <div className="absolute bottom-0 right-0 left-0 px-4 pb-5 pt-8 flex justify-between items-end">
          ...
          <div className="flex flex-col items-center justify-center">
            <p className="text-2xl font-bold text-navy-700">{items}</p>
            <p className="text-sm font-normal text-gray-600">Items</p>
          </div>
          <div className="flex flex-col items-center justify-center">
            <div className="w-20 h-20 bg-white border-4 border-gray-200 rounded-full flex items-center justify-center mb-1">
              <svg
            ...
            <p className="text-sm pt-2 font-semibold text-gray-600 text-center">
              {progress}%
            </p>
          </div>
        </div>
